 ...
 * @date: Mar 17, 2012
 * @file: PostorderTraversalTest.java
 *
 * Description: Test class for postOrderTraversal sub-exercise.
 */
...
public class PostorderTraversalTest
{
    ...
    {
        new PostorderTraversalTest(args[0]);
    }

    public PostorderTraversalTest(String nonce)
    {
        boolean correct = true;
        for (int i = 0; i < 3; i++)
        {
            tree = buildRandomTree();
            PostorderTraversalStudent student = new PostorderTraversalStudent();
            String correctPost = solution.postOrderTraversal(tree);
            String studentPost = student.postOrderTraversal(tree);
            displayVTree(tree);
            System.out.println("Correct Postorder traversal:      " + correctPost);
            System.out.println("Your Postorder traversal: " + studentPost + "\n");
            if (!correctPost.equals(studentPost))
                correct = false;
        }
    ...
    Node buildRandomTree()
    {
        ...
        while (maxDepth(tree) < 5 && solution.postOrderTraversal(tree).length() < 25)
    ...
    {
        String preOrder = solution.postOrderTraversal(tree);
        Random random = new Random();
        char randomKey = (char) (int) (65 + random.nextInt(25));
