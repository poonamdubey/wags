 ...
 * @date: Mar 17, 2012
 * @file: InorderTraversalTest.java
 *
 * Description: Test class for inorderTraversal sub-exercise.
 */
...
public class InorderTraversalTest
{
    ...
    {
        new InorderTraversalTest(args[0]);
    }

    public InorderTraversalTest(String nonce)
    {
        boolean correct = true;
        for (int i = 0; i < 3; i++)
        {
            tree = buildRandomTree();
            InorderTraversalStudent student = new InorderTraversalStudent();
            String correctIn = solution.inOrderTraversal(tree);
            String studentIn = student.inOrderTraversal(tree);
            displayVTree(tree);
            System.out.println("Correct Inorder traversal:      " + correctIn);
            System.out.println("Your inorder traversal: " + studentIn + "\n");
            if (!correctIn.equals(studentIn))
                correct = false;
        }
    ...
    Node buildRandomTree()
    {
        ...
        while (maxDepth(tree) < 5 && solution.inOrderTraversal(tree).length() < 25)
    ...
    {
        String preOrder = solution.inOrderTraversal(tree);
        Random random = new Random();
        char randomKey = (char) (int) (65 + random.nextInt(25));
