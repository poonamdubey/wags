 ...
 * @date: Mar 16, 2012
 * @file: FindRootTest.java
 *
 * Description: The test class for student's findRoot method.
 */
...
public class FindRootTest
{
    ...
    public FindRootTest(String nonce)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            displayVTree(tree);
            char correct = sol.findRoot(testString).getKey();
            char student = ' ';
            if (theirs.findRoot(testString) != null)
                student = theirs.findRoot(testString).getKey();
            System.out.println("Correct Root Node Key: " + correct);
            System.out.println("Your Root Node Key: " + student);
            if (correct != student)
                allRight = false;
        }
